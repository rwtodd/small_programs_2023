...
def out_fn(book,chap)
  "out/#{book[:long]} #{chap} (CLVUL).wikitext".gsub!(' ','_')
end
...
class Converter
  def initialize()
    ...
    @contents = '[[Clementine Vulgate Bible (CLVUL)|Contents]]'
  end
  ...
  def make_chap_link(book, chap, type=:short)
    ...
    sprintf('[[%s %d (CLVUL)|%s %d]]', book[:long], chap, book[type], chap)
  end
  ...
  def generate_prelude(book, bookidx, chap)
    ...
    |1=Clementine Vulgate Bible (CLVUL)
    |2=#{book[:long]} #{chap}
    |3=#{prevlink}
    |4=#{nextlink}}}
    PRELUDE
